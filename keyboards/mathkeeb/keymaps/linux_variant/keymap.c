#include "mathkeeb.h"
// comment to make line numbers match those on localized keymaps
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	KEYMAP(
		...
		KC_TRNS, KC_TRNS, KC_AT, KC_TRNS, KC_TRNS, KC_TRNS, M(11)),
	...
	KEYMAP(
		...
		KC_TRNS, KC_TRNS, KC_AT, KC_TRNS, KC_TRNS, KC_TRNS, M(52)),

	KEYMAP(
		M(75), M(76), KC_X, KC_Y, KC_Z, KC_A, KC_B, KC_C, 
		M(80), KC_LABK, KC_RABK, KC_LBRC, KC_RBRC, M(89), M(90), KC_BSPC, 
		M(92), KC_LPRN, KC_RPRN, M(83), KC_PAST, KC_MINS, KC_PLUS, M(1), 
		M(93), M(3), M(84), M(25), KC_7, KC_8, KC_9, M(2), 
		M(94), M(91), M(77), KC_PERC, KC_4, KC_5, KC_6, KC_EQL, 
		KC_LSFT, KC_CIRC, M(12), M(13), KC_1, KC_2, KC_3, KC_ENT, 
	...
	KEYMAP(
		...
		KC_TRNS, KC_TRNS, KC_AT, KC_TRNS, KC_TRNS, KC_TRNS, M(78)),
...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt) {
	...
	switch (id) {
		...
		case 7:
			if (record->event.pressed) {
				return MACRO( T(L), T(O), T(G), END );
			}
			break;
		case 8:
			if (record->event.pressed) {
				return MACRO( T(L), T(N), END );
			}
			break;
		case 9:
			if (record->event.pressed) {
		...
		case 10:
			if (record->event.pressed) {
				return MACRO( T(E), T(X), T(P), END );
			}
			break;
		case 11:
			if (record->event.pressed) {
				return MACRO( T(D), T(O), T(T), END );
			}
			break;
		case 12:
			if (record->event.pressed) {
		...
		case 15:
			if (record->event.pressed) {
				return MACRO( T(Z), T(E), T(R), T(O), T(S), END );
			}
			break;
		case 16:
			if (record->event.pressed) {
				return MACRO( T(S), T(Q), T(R), T(T), END );
			}
			break;
		case 17:
			if (record->event.pressed) {
				return MACRO( T(S), T(U), T(M), END );
			}
			break;
		case 18:
			if (record->event.pressed) {
				return MACRO( T(P), T(R), T(O), T(D), END );
			}
			break;
		case 19:
			if (record->event.pressed) {
				return MACRO( T(S), T(I), T(N), END );
			}
			break;
		case 20:
			if (record->event.pressed) {
				return MACRO( T(C), T(O), T(S), END );
			}
			break;
		case 21:
			if (record->event.pressed) {
				return MACRO( T(T), T(A), T(N), END );
			}
			break;
		case 22:
			if (record->event.pressed) {
				return MACRO( T(A), T(R), T(C), T(S), T(I), T(N), END );
			}
			break;
		case 23:
			if (record->event.pressed) {
				return MACRO( T(A), T(R), T(C), T(C), T(O), T(S), END );
			}
			break;
		case 24:
			if (record->event.pressed) {
				return MACRO( T(A), T(R), T(C), T(T), T(A), T(N), END );
			}
			break;
		case 25:
			if (record->event.pressed) {
				return MACRO( T(A), T(B), T(S), END );
			}
			break;
		case 26:
			if (record->event.pressed) {
				return MACRO( T(A), T(V), T(G), END );
			}
			break;
		case 27:
			if (record->event.pressed) {
				return MACRO( T(M), T(E), T(A), T(N), END );
			}
			break;
		case 28:
			if (record->event.pressed) {
				return MACRO( T(N), T(O), T(R), T(M), END );
			}
			break;
		case 29:
			if (record->event.pressed) {
		...
		case 39:
			if (record->event.pressed) {
				return MACRO( T(A), T(S), T(I), T(N), END );
			}
			break;
		case 40:
			if (record->event.pressed) {
				return MACRO( T(A), T(C), T(O), T(S), END );
			}
			break;
		case 41:
			if (record->event.pressed) {
				return MACRO( T(A), T(T), T(A), T(N), END );
			}
			break;
		case 42:
			if (record->event.pressed) {
		...
		case 44:
			if (record->event.pressed) {
				return MACRO( T(I), T(N), T(V), END );
			}
			break;
		case 45:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(L), T(O), T(G), T(1), T(0), END );
			}
			break;
		case 46:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(L), T(O), T(G), END );
			}
			break;
		case 47:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(E), T(X), T(P), END );
			}
			break;
		case 48:
			if (record->event.pressed) {
		...
		case 52:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(D), T(O), T(T), END);
			}
			break;
		case 53:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(S), T(Q), T(R), T(T), END );
			}
			break;
		case 54:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(S), T(U), T(M), END );
			}
			break;
		case 55:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(P), T(R), T(O), T(D), END );
			}
			break;
		case 56:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(S), T(I), T(N), END );
			}
			break;
		case 57:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(C), T(O), T(S), END );
			}
			break;
		case 58:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(T), T(A), T(N), END );
			}
			break;
		case 59:
			if (record->event.pressed) {
		...
		case 62:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(A), T(B), T(S), END );
			}
			break;
		case 63:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(L), T(I), T(N), T(A), T(L), T(G), T(DOT), T(N), T(O), T(R), T(M), END );
			}
			break;
		case 64:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(M), T(E), T(A), T(N), END );
			}
			break;
		case 65:
			if (record->event.pressed) {
		...
		case 67:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(A), T(R), T(R), T(A), T(Y), END);
			}
			break;
		case 68:
			if (record->event.pressed) {
		...
		case 69:
			if (record->event.pressed) {
				return MACRO( T(L), T(O), T(G), T(1), T(0), END );
			}
			break;
		case 70:
			if (record->event.pressed) {
		...
		case 75:
			if (record->event.pressed) {
				return MACRO( T(BSLS), T(S), T(U), T(M), END );
			}
			break;
		case 76:
			if (record->event.pressed) {
				return MACRO( T(BSLS), T(P), T(R), T(O), T(D), END );
			}
			break;
		case 77:
			if (record->event.pressed) {
				return MACRO( T(BSLS), T(S), T(Q), T(R), T(T), END );
			}
			break;
		case 78:
			if (record->event.pressed) {
		...
		case 81:
			if (record->event.pressed) {
				return MACRO( T(E), D(LSFT), T(6), U(LSFT), END );
			}
			break;
		case 82:
			if (record->event.pressed) {
		...
		case 98:
			if (record->event.pressed) {
				return MACRO( T(BSLS), D(LSFT), T(S), U(LSFT), T(I), T(G), T(M), T(A), END);
			}
			break;
		case 99:
			if (record->event.pressed) {
				return MACRO( T(BSLS), D(LSFT), T(P), U(LSFT), T(I), END);
			}
			break;
		case 100:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(Z), T(E), T(R), T(O), T(S), END);
			}
			break;
	}
	return MACRO_NONE;
