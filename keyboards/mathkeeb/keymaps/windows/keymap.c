...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt) {
	...
	switch (id) {
		...
		case 11:
			if (record->event.pressed) {
				return MACRO( T(D), T(O), T(T), D(LSFT), T(9), U(LSFT), END );
			}
			break;
		case 12:
			if (record->event.pressed) {
		...
		case 15:
			if (record->event.pressed) {
				return MACRO( T(Z), T(E), T(R), T(O), T(S), D(LSFT), T(9), U(LSFT), END );
			}
			break;
		case 16:
			if (record->event.pressed) {
		...
		case 59:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(A), T(R), T(C), T(S), T(I), T(N), D(LSFT), T(9), U(LSFT), END );
			}
			break;
		case 60:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(A), T(R), T(C), T(C), T(O), T(S), D(LSFT), T(9), U(LSFT), END );
			}
			break;
		case 61:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(A), T(R), T(C), T(T), T(A), T(N), D(LSFT), T(9), U(LSFT), END );
			}
			break;
		case 62:
			if (record->event.pressed) {
		...
		case 83:
			if (record->event.pressed) {
				return MACRO( T(BSLS), T(F), T(R), T(A), T(C), D(LSFT), T(LBRC), T(RBRC), T(LBRC), T(RBRC), U(LSFT), T(LEFT), T(LEFT), T(LEFT), END );
			}
			break;
		case 84:
			if (record->event.pressed) {
				return MACRO( T(BSLS), T(B), T(A), T(R), D(LSFT), T(LBRC), U(LSFT), END );
			}
			break;
		case 85:
			if (record->event.pressed) {
		...
		case 100:
			if (record->event.pressed) {
				return MACRO( T(N), T(P), T(DOT), T(Z), T(E), T(R), T(O), T(S), D(LSFT), T(9), U(LSFT), END);
			}
			break;
	}
	return MACRO_NONE;
