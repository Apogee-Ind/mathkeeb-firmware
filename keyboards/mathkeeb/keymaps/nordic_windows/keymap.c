...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	KEYMAP(
		...
		M(80), NO_LESS, NO_GRTR, NO_LBRC, NO_RBRC, M(7), M(8), KC_BSPC, 
		M(19), NO_LPRN, NO_RPRN, M(72), M(78), NO_MINS, NO_PLUS, M(1), 
		M(20), M(3), M(84), M(25), KC_7, KC_8, KC_9, M(2), 
		M(21), M(14), M(77), KC_PERC, KC_4, KC_5, KC_6, NO_EQL, 
		KC_LSFT, NO_CIRC, M(12), M(13), KC_1, KC_2, KC_3, KC_ENT, 
...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt) {
	...
	switch (id) {
		...
		case 83:
			if (record->event.pressed) {
				return MACRO( D(RALT), T(MINS), U(RALT), T(F), T(R), T(A), T(C), END );
			}
			break;
		case 84:
			if (record->event.pressed) {
				return MACRO( D(RALT), T(MINS), U(RALT), T(B), T(A), T(R), END );
			}
			break;
		case 85:
			if (record->event.pressed) {
